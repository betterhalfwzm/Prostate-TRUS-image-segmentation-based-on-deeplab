      ...
      with tf.contrib.slim.arg_scope(resnet_v2.resnet_arg_scope(batch_norm_decay=batch_norm_decay)):
        with arg_scope([layers.batch_norm], is_training=is_training):
          with tf.variable_scope("low_level_features"):
              ...
              low_level_features = layers_lib.conv2d(low_level_features, 32, [1, 1], stride=1, scope='conv_1x1')
              low_level_features_size = tf.shape(low_level_features)[1:3]

              low_level_features1 = end_points[base_architecture + '/block2/unit_4/bottleneck_v2/conv1']
              low_level_features1 = layers_lib.conv2d(low_level_features1, 32, [1, 1], stride=1, scope='conv_1x1_1')
              low_level_features_size1 = tf.shape(low_level_features1)[1:3]

              low_level_features2 = end_points[base_architecture + '/block3/unit_23/bottleneck_v2/conv1']
              low_level_features2 = layers_lib.conv2d(low_level_features2, 32, [1, 1], stride=1, scope='conv_1x1_2')
              low_level_features_size2 = tf.shape(low_level_features2)[1:3]

              low_level_features3 = end_points[base_architecture + '/block4/unit_3/bottleneck_v2/conv1']
              low_level_features3 = layers_lib.conv2d(low_level_features3, 32, [1, 1], stride=1, scope='conv_1x1_3')
              low_level_features_size3 = tf.shape(low_level_features3)[1:3]

          with tf.variable_scope("upsampling_logits"):

              net = tf.image.resize_bilinear(encoder_output, low_level_features_size, name='upsample_0')
              #net = layers_lib.conv2d(net, 256, [3, 3], stride=1, scope='conv_3x3_0')
              net = tf.concat([net, low_level_features], axis=3, name='concat')

              net1 = tf.image.resize_bilinear(net, low_level_features_size1, name='upsample_1')
              #net1 = layers_lib.conv2d(net1, 256, [3, 3], stride=1, scope='conv_3x3_1')
              net1 = tf.concat([net1, low_level_features1], axis=3, name='concat')

              net2 = tf.image.resize_bilinear(net1, low_level_features_size2, name='upsample_2')
              #net2 = layers_lib.conv2d(net2, 256, [3, 3], stride=1, scope='conv_3x3_2')
              net2 = tf.concat([net2, low_level_features2], axis=3, name='concat')

              net3 = tf.image.resize_bilinear(net2, low_level_features_size3, name='upsample_3')
              #net3 = layers_lib.conv2d(net3, 256, [3, 3], stride=1, scope='conv_3x3_3')
              net3 = tf.concat([net3, low_level_features3], axis=3, name='concat')

              net = layers_lib.conv2d(net3, 256, [3, 3], stride=1, scope='conv_3x3_1')
              net = layers_lib.conv2d(net, 256, [3, 3], stride=1, scope='conv_3x3_2')
              net = layers_lib.conv2d(net, num_classes, [1, 1], activation_fn=None, normalizer_fn=None,scope='conv_1x1')
              logits = tf.image.resize_bilinear(net, inputs_size, name='upsample_5')
...
def deeplabv3_plus_model_fn(features, labels, mode, params):
  ...
  #T = tf.metrics._confusion_matrix_at_thresholds(valid_labels, valid_preds, 0.9)
